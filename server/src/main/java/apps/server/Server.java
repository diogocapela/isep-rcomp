...
class Server {
    static String APP_NAME = "Server";
    static InetAddress broadcastAddress;
    static DatagramSocket datagramSocket;

    static String SHARED_HTTP_DATA = "This is HTTP data!";
    ...
    public static void main(String args[]) throws Exception {
        ...
        Thread httpServer = new Thread(new HTTPServer(SHARED_HTTP_DATA));
        Thread udpReceiver = new Thread(new ServerReceive(datagramSocket));
        httpServer.start();
        udpReceiver.start();
        while (true) {
            command = in.readLine();
            if (command.compareTo("EXIT") == 0) {
                ...
                if(appToComunicate.equalsIgnoreCase(APP_NAME)){
                    ...
                    if ("STORE".equalsIgnoreCase(command.split(" ")[1])) {
                        ServerReceive.storeMapa(command.split(" ")[2], command.split(" ")[3]);
                        System.out.println("Store successful");

                    } else if ("LIST".equalsIgnoreCase(command.split(" ")[1])) {
                        ServerReceive.listMapa();

                    } else if ("FETCH".equalsIgnoreCase(command.split(" ")[1])) {
                        System.out.println(ServerReceive.fetchFromMap(command.split(" ")[2]));

                    } else if ("ERASE".equalsIgnoreCase(command.split(" ")[1])) {
                        ServerReceive.eraseFromMapa(command.split(" ")[2]);
                        System.out.println("Store successful");
                    }
                }else {
...
class ServerReceive implements Runnable {
    ...
    private String APP_NAME = "Server";

    public ServerReceive(DatagramSocket datagramSocket) {
...
class HTTPServer implements Runnable {
    ...
    private String ownHTTPData;

    public HTTPServer(String ownHTTPData) {
        this.ownHTTPData = ownHTTPData;
    }

    @RequestMapping("/")
    String home() {
        return "<h1>Hello World!" + ownHTTPData + "</h1>";
    }

    @Override
    public void run() {
        System.out.println("RUNNING");
        SpringApplication.run(http.HTTPServer.class);
    }
}
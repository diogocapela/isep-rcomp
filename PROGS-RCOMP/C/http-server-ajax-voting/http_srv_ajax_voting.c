...
int main(int argc, char **argv) {
    ...
    if (argc != 2) {
        puts("Oops, local TCP port number missing from command line");
        exit(1);
    }

    // Preenche a estrutura com zeros (initial state)
    ...
    // Associar o socket à porta/estrutura
...
void processHttpRequest(int sock, int conSock) {
    ...
    if (!strncmp(requestLine, "PUT /votes/", 11))
        processPUT(conSock, requestLine);
    else {
